setwd("~/Dropbox/Cambridge/PhD/ICAR/AIG/Map/html")

===============================================================================================
  require(netGen)
require(igraph)

#sample<- read.csv("sample2.csv",check.names=FALSE,row.names=1)
#sample<- read.csv("sample_adjmatrix.csv",row.names=1,check.names=FALSE)
m=as.matrix(sample)
g=graph.adjacency(m,mode="undirected",weighted=NULL,diag=FALSE)
p <- tkplot(g)

j.coord <- tkplot.getcoords(p)
j.coord
...
o <- closedMaps(E.trail ,base.colour=3, start.colour=9,end.colour= 4,z=NULL,newValue=9,default.colour=FALSE, edge.value=0, no.label=FALSE)

#Must normalise coordinates
#grconvertX and gconvertY create the right pixel coordinates.
o<- g
coordinates.1 <- layout.norm(j.coord)
...
==============================================================================
  ...
  jsfile = file.path('~/Dropbox/Cambridge/PhD/ICAR/AIG/Map/html/', "script.js")
...
cat("function linedraw(x1, y1, x2, y2, lineid){
    if(y1 < y2){
    var pom = y1;
    y1 = y2;
    y2 = pom;
...
cat(edge.list, append=TRUE, file=jsfile)
